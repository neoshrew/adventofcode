...
object solution {
  ...
  def getPointsBetween(startCoord: Coord, endCoord: Coord): IterableOnce[(Int, Int)] = {
    ...
    val dyIter = (startY to endY by (if (startY > endY) -1 else 1))
    val dxIter = (startX to endX by (if (startX > endX) -1 else 1))
    if (startX == endX) {
      for (dY <- dyIter ) yield (startX, dY)
    } else if (startY == endY) {
      for (dX <- dxIter) yield (dX, startY)
    } else {
      for (coord <- dxIter.zip(dyIter)) yield coord
    }
  }
