...
object solution {
  ...
  val countCache = MutableMap[Tuple2[Tuple2[Char, Char],Int],Map[Char,Long]]()
  ...
  def main(args: Array[String]): Unit = {
    ...
    val counts = addCounts(
      badCounts,
      startingPattern.slice(1, startingPattern.length-1).map(_ -> -1L).toMap
    )   
  ...
  def getCounts(transforms: Map[Tuple2[Char, Char], Char],
    ...
    if (!countCache.contains(cacheKey)) {
      if (nSteps > 0 && transforms.contains(pair)) {
        ...
        if (pair._1 == pair._2) {
          countCache(cacheKey) = Map(pair._1->2L)
        } else {
          countCache(cacheKey) = Map(pair._1->1L, pair._2->1L)
        }
      }
    }
    countCache(cacheKey)
  ...
  def addCounts(a: Map[Char, Long], b: Map[Char, Long]): Map[Char, Long] = {
    a.keys.toSet.union(b.keys.toSet).map(
      char => (char, a.getOrElse(char, 0L)+b.getOrElse(char, 0L))
    ).toMap
  }
